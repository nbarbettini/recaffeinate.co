...
A REST API might return JSON or XML instead, but the principle is the same. Here's what a JSON response might look like, modeled with [Ion](/post/modeling-json-api-with-ion/):
...
A simple root document could look like (using [Ion](/post/modeling-json-api-with-ion/) again):
